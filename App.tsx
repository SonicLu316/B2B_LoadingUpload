...
const App = () => {
  ...
  const filteredRecords = useMemo(() => {
    return containerRecords.filter(r => r.poNumber.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        <header className="bg-white border-b p-4 sticky top-0 z-40 shadow-sm">
          <div className="max-w-2xl mx-auto flex items-center justify-between">
            <button 
              ...
              className="flex items-center gap-1 text-slate-500 font-bold text-sm hover:text-red-500 transition-colors"
            >
              <X size={20} /> 取消新增
            </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    className={`w-full bg-slate-50 border-2 rounded-2xl p-4 focus:border-blue-500 focus:bg-white outline-none transition-all font-mono ${!workingData.poNumber ? 'border-red-50' : 'border-slate-50'}`}
                ...
                <div ref={shippingRef} className="space-y-1.5">
                  ...
                  <input 
                    placeholder="出貨地點"
                    className={`w-full bg-slate-50 border-2 rounded-2xl p-4 focus:border-blue-500 focus:bg-white outline-none transition-all ${!workingData.shippingLocation ? 'border-red-50' : 'border-slate-50'}`}
                  ...
                  <div key={step.id} ref={el => { stepRefs.current[step.id] = el }} className={`p-4 rounded-3xl border-2 transition-all duration-500 ${unlocked ? 'bg-white border-white shadow-sm' : 'bg-slate-200/40 border-transparent opacity-60'}`}>
                    ...
                    {unlocked && (
                      <div className="space-y-4 animate-in fade-in slide-in-from-top-1 duration-500">
                        ...
                        <input 
                          placeholder="新增備註說明..." 
                          className="bg-slate-50 w-full p-4 rounded-2xl text-sm outline-none border-2 border-transparent focus:border-blue-100 focus:bg-white transition-all" 
